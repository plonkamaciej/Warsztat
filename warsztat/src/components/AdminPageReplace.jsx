...
export const AdminPageReplace = () => {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:8080/addCarForReplace', newCar)
            .then(response => {
                setNewCar({
                    id: '',
                    mark: '',
    ...
    return (
        ...
        <h3>Dodaj nowe auto:</h3>
            ...
            <label>
                Lokalizacja:
                <input type="text" name="location" className='w-full block m-1 px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400
            focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500
            disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
            invalid:border-pink-500 invalid:text-pink-600
            focus:invalid:border-pink-500 focus:invalid:ring-pink-500' value={newCar.location} onChange={handleInputChange} />
            </label>
            <label>
                Date of Production:
